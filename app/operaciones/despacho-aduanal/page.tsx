...
const aduanas = ["Nuevo Laredo", "Manzanillo", "Veracruz", "Altamira", "CDMX AICM"];
...
const checklistPorOperacion: Record<TipoOperacion, string[]> = {
  IMPO: ["Factura", "BL (Bill of Lading)", "Packing List (PL)", "Certificado de Origen (CO)", "Permisos", "Hoja Tecnica"],
...
function crearEstadoInicial() {
  ...
  if (typeof window === "undefined") {
    ...
    return {
      ...
      historial: [{ id: 1, mensaje: `Operacion creada con folio ${folio}`, fecha: fechaActual() }] as HistorialCambio[],
    };
  }
...
export default function DespachoAduanalPage() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ operacion, pasos, documentos, historial }));
  ...
  const pasosFiltrados = pasos.filter((paso) => {
    ...
    return paso.etapa.toLowerCase().includes(term) || estadoLabel[paso.estado].toLowerCase().includes(term);
  });
  ...
  const agregarHistorial = (mensaje: string) => {
    setHistorial((prev) => {
      ...
      return [{ id: nextId, mensaje, fecha: fechaActual() }, ...prev].slice(0, 20);
    });
  };

  const handleCambioOperacion = <K extends keyof EstadoOperacion>(campo: K, valor: EstadoOperacion[K]) => {
  ...
  const handleEstadoPaso = (id: number, estado: PasoEstado) => {
    setPasos((prev) =>
      prev.map((paso) => {
        ...
        return { ...paso, estado, fecha: estado === "pendiente" ? "-" : fechaCortaISO() };
      })
    );
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <Link href="/operaciones" className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors text-sm font-medium">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6 border-b border-slate-100">
          <div>
            ...
            <input type="text" value={operacion.clienteNombre} onChange={(e) => handleCambioOperacion("clienteNombre", e.target.value)} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm" />
          </div>
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wide">Tipo de Cliente</p>
            <select value={operacion.clienteTipo} onChange={(e) => handleCambioOperacion("clienteTipo", e.target.value as TipoCliente)} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm">
          ...
          <div>
            ...
            <input type="text" value={operacion.rfc} onChange={(e) => handleCambioOperacion("rfc", e.target.value.toUpperCase())} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm" />
          </div>
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wide">Aduana</p>
            <select value={operacion.aduana} onChange={(e) => handleCambioOperacion("aduana", e.target.value)} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm">
              {aduanas.map((aduana) => (
                <option key={aduana} value={aduana}>{aduana}</option>
              ))}
            </select>
          </div>
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wide">Tipo de Operacion</p>
            <select value={operacion.tipoOperacion} onChange={(e) => handleTipoOperacion(e.target.value as TipoOperacion)} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm">
              <option value="IMPO">IMPO</option>
              <option value="EXPO">EXPO</option>
            </select>
          </div>
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wide">{operacion.tipoOperacion === "IMPO" ? "ETA" : "ETD"}</p>
            <input type="date" value={operacion.tipoOperacion === "IMPO" ? operacion.eta : operacion.etd} onChange={(e) => operacion.tipoOperacion === "IMPO" ? handleCambioOperacion("eta", e.target.value) : handleCambioOperacion("etd", e.target.value)} className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30 text-sm" />
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs px-3 py-1 rounded-lg bg-aduanaBlue/10 text-aduanaBlue border border-aduanaBlue/20 font-medium">{checklistCompletado}/{checklistActual.length} documentos</span>
                </div>
                    ...
                    return (
                      <div key={doc} className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 px-3 py-3 border border-slate-200 rounded-lg">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{cargado ? `Archivo: ${cargado}` : "Sin archivo cargado"}</p>
                        </div>
                        <div>
                          <input id={inputId} type="file" className="hidden" onChange={(e) => handleUploadDocumento(doc, e.target.files?.[0] ?? null)} />
                          <label htmlFor={inputId} className="inline-flex cursor-pointer items-center gap-2 px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex flex-col md:flex-row gap-3 items-center md:justify-between bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-1">
                <div className="relative w-full md:w-80 lg:w-96">
                  ...
                  <input type="text" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Filtrar etapa o estado..." className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-aduanaBlue/15 focus:border-aduanaBlue/30" />
                </div>
                <span className="text-xs px-3 py-2 rounded-lg bg-aduanaBlue/10 text-aduanaBlue border border-aduanaBlue/20 font-medium">Workflow activo</span>
              </div>
                      ...
                      <tr key={paso.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm">
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${estadoStyles[paso.estado]}`}>{estadoLabel[paso.estado]}</span>
                            <select value={paso.estado} onChange={(e) => handleEstadoPaso(paso.id, e.target.value as PasoEstado)} className="px-2 py-1 border border-slate-200 rounded-lg text-xs text-slate-600">
            ...
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                ...
                <p className="text-sm text-slate-600"><strong>{progreso}%</strong> completado</p>
                <p className="text-xs text-slate-500 mt-1">Etapas completadas: {completados} de {pasos.length}</p>
              </div>
